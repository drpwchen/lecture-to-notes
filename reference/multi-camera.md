...
- [每個裝置各有一個時鐘，而且都是錯的](#device-clock-calibration)
  - [沒有時戳的素材，鄰號檔案可以把它夾出來](#serial-bracketing)
  - [交叉相關會匹配「同一份講稿」，不只匹配「同一次錄影」](#xcorr-same-talk-trap)
  - [相機時鐘錯誤會生出一個不存在的年份](#phantom-year)
- [流程](#flow)
- [多來源去重](#dedup)
- [轉檔與檔名](#transcode)
- [網頁](#web)
...
有些檔案任何時鐘都讀不到（本例：兩支 `MVI_*.MP4`，容器沒有拍攝時間）。==不要因此
放棄——同一張記憶卡的流水號本身就是順序證據。==

Canon 這類相機的靜照與影片**共用同一組流水號**，所以一支無時戳的 `MVI_9953` 夾在
`IMG_9952` 與 `IMG_9955` 之間，而靜照有 EXIF：兩張的時間（14:21:06 與 14:31:24）
就把影片框進一個十分鐘的窗口。再用影片自身長度做算術（9953 長 3:05、9954 長 1:37）
即可把兩支排進窗口內且與流水號一致。

配合低信心交叉相關使用：xcorr 對該片只拿到 10 票（低於門檻、單獨不可採信），但它算出
的 14:25:30 落在夾擠窗口內 ⟹ ==兩條互相獨立的線索一致，才可採用==。同一批裡另一支
只有 2 票的結果與流水號順序矛盾，判為雜訊捨棄——==低票數不是「比較不準」，是不能用==。

### 交叉相關會匹配「同一份講稿」，不只匹配「同一次錄影」 {#xcorr-same-talk-trap}

`xcorr_media_offsets.py` 投票的是共同 N-gram。==講者第二年再講一次同一套教材，逐字稿
會有大量共同 N-gram，於是它一樣給出高票數與收斂的偏移。== 高信心不等於同一次錄影。

==**先看離散，不要先看票數。**== 同一場活動雙裝置錄下來，各支素材回推的時鐘偏移應該
落在**數秒**之內（本檔上面那個校準實例＝±3 秒）。==離散到數十秒，就是「不同場次」的
指紋==——現場講授的節奏每年都會漂。實例（2026-08-05）：19 支素材回推的偏移離散 45 秒，
判讀者拿它去跟「4.5 年的時鐘誤差」相比，覺得「才 45 秒、很準」，於是接受了一個完全
錯誤的結論。==離散要跟「幾秒」這個絕對標準比，不要跟你正在證明的那個大數字比。==

分辨方法是找**只有同一次錄影才會有的東西**，而不是看票數：

- **場內的一次性言語**——臨場提到別位講者（「just like what Dr. &lt;speaker&gt; showed us」）、
  回答現場提問、講錯又更正。這些在另一次講授不會逐字重現。
- **雙裝置的交錯覆蓋**：同一場活動的兩台裝置應該**部分重疊且起訖錯開**（一台從開場
  錄、另一台中途才開）。若兩份逐字稿是整段對齊的複製關係，那是同一支檔案被轉錄兩次，
  不是兩台裝置。
- ==**用類別型證據，不要用相似度分數。**== 判「是不是同一批檔案」，看**時長是否秒秒
  相同、檔案大小是否完全相同**——同一場講授錄兩次不可能秒數全同。實例：19/19 時長
  與大小完全一致 ⟹ 同一批檔案，一句話定案。反之逐字稿相似度落在 0.71–0.86，而
  「同一支檔案跑兩次 ASR」與「同一份講稿講兩年」==兩者的分數區間完全重疊，分不開==。
- ⚠️ `difflib.quick_ratio()` 只比字元多重集，是相似度的**上界近似**，不是內容比對；
  同語言同主題的逐字稿天然就有 0.96+。要比就用 `ratio()`，而且要先剝掉時間戳、
  比**字序**——但即使如此，仍見上一條：分數不是類別證據。

### 相機時鐘錯誤會生出一個不存在的年份 {#phantom-year}

==資料夾名若來自素材時戳，時鐘錯了就會憑空長出一個看似合理的年份==，而且此後每一份
衍生產物都繼承它。實例：一批 2020-12-19 的錄影因相機時鐘慢了 4 年 5 個月而被標成
2016，資料夾、HUB、交付網頁全部叫「2016 workshop」——而該講者確實另有一場真的 2016
同主題課程，於是兩者從名稱上完全無法分辨，交付物也就無從判斷描述的是哪一場。

批次入庫時值得做一項機械檢查：==不同課程資料夾中出現檔名相同、大小相同的素材==
（只比對檔名與 size，不讀內容、不 hash），命中就人工判定是「複本」還是「兩場獨立
錄影」。它便宜，而且能在一整批衍生產物長出來之前攔下。
